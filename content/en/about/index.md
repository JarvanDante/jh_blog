...
1、主导 jh 从“单体思维”向“领域化微服务”演进，拆分 `jh_gateway / jh_app_service / jh_game_service / jh_balance_service / jh_manage_service`，并按 `/api/frontend`、`/api/backend`、`/api/manage` 统一入口路由，降低模块耦合与发布风险。
2、建设网关治理能力：统一鉴权、限流、错误翻译、路由分发与站点域名解析；推动 Nginx 反代标准化（Host 透传、history fallback、upstream 规范），显著减少 502 与路由错配问题。
3、落地 Outbox + MQ 最终一致性链路，拆分常驻消费者与定时补偿任务（relay 常驻 + cron 兜底），解决“消息发出但未落地”的一致性风险，提升资金与游戏链路可靠性。
4、完成 MySQL -> ClickHouse 分析链路治理，针对 `ch_failed` 场景建立“指标观测 -> 日志定位 -> schema 对齐 -> 回归验证”闭环，修复字段缺失导致的批量落库失败问题。
5、推进前端稳定性改造（jh_h5）：修复 `/rebate` 黑屏（缺字段导致格式化崩溃），推动“前端容错 + 网关 EmitUnpopulated”双保险；完善 loading 收口、懒加载错误自愈、调试开关与反调试开关。
6、推动部署策略从 ACK 向低成本 Docker Compose 迁移，形成固定 tag 发布、分批升级、快速回滚 SOP；沉淀镜像架构一致性（amd64/arm64）与配置挂载规范，提升上线稳定性与成本效率。
7、完善可观测与排障体系：结合 EFK、Jaeger、服务日志与链路追踪，形成四层排障方法（入口层/网关层/发现层/业务层），把“临时修复”沉淀为可复用的工程机制。
8、主导后台安全能力建设：员工/管理员 Google2FA 全链路接入（登录校验、状态管理、错误透传），并统一总后台与站点后台权限模型，提升管理面安全基线。

#### jh 项目提炼（项目亮点）

- **架构能力**：能在业务持续迭代中完成服务拆分、边界治理与链路收口，而不是一次性重写。
- **故障处理能力**：具备从指标到日志、从现象到根因的快速定位能力，能在高压场景下先恢复可用性再做根因治理。
- **工程化能力**：把发布、回滚、配置、观测、补偿机制流程化，确保系统可长期稳定演进。
- **协同能力**：能同时推进后端、网关、前端、运维多角色协同，保障改动最终在生产可落地。
