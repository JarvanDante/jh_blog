---
author: "jarvan"
title: "MinIO 对象存储：上传资源与静态访问链路"
date: 2025-08-14T10:00:00+08:00
description: "围绕 minio 的定位、关键配置、使用方式与服务通信关系，给出可复用的实战说明。"
draft: false
hideToc: false
enableToc: true
...
- **端口**：`19000,19001`
- **配置挂载/核心参数**：`MINIO_ROOT_USER/PASSWORD`
...
`services -> minio, nginx /uploads proxy`
...
```bash
# 在同网络容器内验证目标服务
docker exec -it nginx sh -c "ping -c 1 minio || true"
```
...
`minio` 不是孤立容器，而是整套微服务链路中的一个职责节点。只有把“作用、配置、通信、排障”四件事标准化，系统才能在迭代中保持稳定。
