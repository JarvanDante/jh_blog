...
为了让 `clickhouse` 在生产长期稳定运行，我会在每次上线前做这份 checklist：
...
```bash
docker exec -it clickhouse sh -c "ls -lah /app || true"
```

4. 验证关键依赖连通：

```bash
docker exec -it clickhouse sh -c "getent hosts mysql rabbitmq redis consul || true"
```
...
针对 `clickhouse`，我通常会从下面几个维度做容量管理：
...
1. 为 `clickhouse` 增加更细粒度的健康探针与自动告警。
