...
为了让 `mysql` 在生产长期稳定运行，我会在每次上线前做这份 checklist：
...
```bash
docker exec -it mysql sh -c "ls -lah /app || true"
```

4. 验证关键依赖连通：

```bash
docker exec -it mysql sh -c "getent hosts mysql rabbitmq redis consul || true"
```
