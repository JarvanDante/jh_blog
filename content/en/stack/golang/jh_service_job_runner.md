...
为了让 `job_runner` 在生产长期稳定运行，我会在每次上线前做这份 checklist：
...
```bash
docker exec -it job_runner sh -c "ls -lah /app || true"
```

4. 验证关键依赖连通：

```bash
docker exec -it job_runner sh -c "getent hosts mysql rabbitmq redis consul || true"
```
...
```bash
# 示例：改回上一个 tag 后重建
# docker compose up -d --force-recreate job_runner
```
...
针对 `job_runner`，我通常会从下面几个维度做容量管理：
...
1. 为 `job_runner` 增加更细粒度的健康探针与自动告警。
