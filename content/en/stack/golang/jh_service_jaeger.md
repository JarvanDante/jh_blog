---
author: "jarvan"
title: "Jaeger 链路追踪：跨服务请求路径可观测实践"
date: 2025-07-20T10:00:00+08:00
description: "围绕 jaeger 的定位、关键配置、使用方式与服务通信关系，给出可复用的实战说明。"
draft: false
hideToc: false
enableToc: true
...
- **端口**：`16686, 4317`
- **配置挂载/核心参数**：`COLLECTOR_OTLP_ENABLED`
...
`services(otel) -> jaeger collector`
...
```bash
# 在同网络容器内验证目标服务
docker exec -it nginx sh -c "ping -c 1 jaeger || true"
```
...
`jaeger` 不是孤立容器，而是整套微服务链路中的一个职责节点。只有把“作用、配置、通信、排障”四件事标准化，系统才能在迭代中保持稳定。
