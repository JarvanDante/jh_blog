...
为了让 `fluent_bit` 在生产长期稳定运行，我会在每次上线前做这份 checklist：
...
```bash
docker compose ps fluent_bit
```

2. 看最近 200 行关键日志：

```bash
docker logs --tail=200 fluent_bit
```

3. 验证配置是否被正确挂载：

```bash
docker exec -it fluent_bit sh -c "ls -lah /app || true"
```

4. 验证关键依赖连通：

```bash
docker exec -it fluent_bit sh -c "getent hosts mysql rabbitmq redis consul || true"
```
...
```bash
# 示例：改回上一个 tag 后重建
# docker compose up -d --force-recreate fluent_bit
```
...
针对 `fluent_bit`，我通常会从下面几个维度做容量管理：
...
1. 为 `fluent_bit` 增加更细粒度的健康探针与自动告警。
