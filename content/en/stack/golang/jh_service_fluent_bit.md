---
author: "jarvan"
title: "Fluent Bit 日志采集：Docker 多容器日志汇聚实战"
date: 2025-07-15T10:00:00+08:00
description: "围绕 fluent-bit 的定位、关键配置、使用方式与服务通信关系，给出可复用的实战说明。"
draft: false
hideToc: false
enableToc: true
...
在当前编排里，`fluent-bit` 重点关注：

- **端口**：`24224`
- **配置挂载/核心参数**：`fluent-bit.conf, parsers.conf`
...
`docker logs -> fluent-bit -> elasticsearch`
...
```bash
docker logs --tail=200 fluent-bit
```
...
```bash
# 在同网络容器内验证目标服务
docker exec -it nginx sh -c "ping -c 1 fluent-bit || true"
```
...
`fluent-bit` 不是孤立容器，而是整套微服务链路中的一个职责节点。只有把“作用、配置、通信、排障”四件事标准化，系统才能在迭代中保持稳定。
