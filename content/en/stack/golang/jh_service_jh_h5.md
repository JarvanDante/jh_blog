---
author: "jarvan"
title: "jh_h5 前台站点容器：用户触达与业务入口页面"
date: 2025-09-18T10:00:00+08:00
description: "围绕 jh_h5 的定位、关键配置、使用方式与服务通信关系，给出可复用的实战说明。"
draft: false
hideToc: false
enableToc: true
...
`browser -> nginx -> jh_h5 + /api -> gateway`
...
```bash
docker compose up -d jh_h5
docker compose ps jh_h5
```

### 日志检查

```bash
docker logs --tail=200 jh_h5
```
...
```bash
# 在同网络容器内验证目标服务
docker exec -it nginx sh -c "ping -c 1 jh_h5 || true"
```
...
`jh_h5` 不是孤立容器，而是整套微服务链路中的一个职责节点。只有把“作用、配置、通信、排障”四件事标准化，系统才能在迭代中保持稳定。


## 7. 生产级运行清单（SRE Checklist）

为了让 `jh_h5` 在生产长期稳定运行，我会在每次上线前做这份 checklist：
...
```bash
docker exec -it jh_h5 sh -c "ls -lah /app || true"
```

4. 验证关键依赖连通：

```bash
docker exec -it jh_h5 sh -c "getent hosts mysql rabbitmq redis consul || true"
```
...
```bash
# 示例：改回上一个 tag 后重建
# docker compose up -d --force-recreate jh_h5
```
