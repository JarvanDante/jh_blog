...
为了让 `consul` 在生产长期稳定运行，我会在每次上线前做这份 checklist：
...
```bash
docker exec -it consul sh -c "ls -lah /app || true"
```

4. 验证关键依赖连通：

```bash
docker exec -it consul sh -c "getent hosts mysql rabbitmq redis consul || true"
```
...
```bash
# 示例：改回上一个 tag 后重建
# docker compose up -d --force-recreate consul
```
