---
author: "jarvan"
title: "jh_gateway 统一业务网关：鉴权、路由、限流与错误翻译"
date: 2025-08-24T10:00:00+08:00
description: "围绕 jh_gateway 的定位、关键配置、使用方式与服务通信关系，给出可复用的实战说明。"
draft: false
hideToc: false
enableToc: true
...
在当前编排里，`jh_gateway` 重点关注：

- **端口**：`8100`
- **配置挂载/核心参数**：`/app/config.yaml, CONSUL_ADDR`
...
`nginx -> gateway -> app/game/balance/manage gRPC`
...
```bash
docker compose up -d jh_gateway
docker compose ps jh_gateway
```
...
```bash
# 在同网络容器内验证目标服务
docker exec -it nginx sh -c "ping -c 1 jh_gateway || true"
```
...
`jh_gateway` 不是孤立容器，而是整套微服务链路中的一个职责节点。只有把“作用、配置、通信、排障”四件事标准化，系统才能在迭代中保持稳定。
