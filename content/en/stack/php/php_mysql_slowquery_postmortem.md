---
author: "jarvan"
title: "一次线上慢查询复盘：MySQL 索引、SQL 重写与容量治理"
date: 2020-07-18T10:00:00+08:00
description: "一次线上慢查询复盘：MySQL 索引、SQL 重写与容量治理"
draft: false
hideToc: false
enableToc: true
...
线上某天 P95 从 180ms 飙到 2.4s，最终定位是一个运营报表 SQL 把交易库拖慢。

## 事故现场

- 慢 SQL：`where created_at between ... and ... order by id desc limit 1000`
- 表规模：千万级
- 索引：只有单列 `created_at`
...
1. 新增复合索引 `(site_id, created_at, id)`
2. 改 SQL，避免无谓列扫描
3. 报表查询拆到只读副本

```sql
ALTER TABLE orders
ADD INDEX idx_site_created_id (site_id, created_at, id);
```
...
“慢查询不是 DBA 的问题”，它是业务、应用、数据库三方共同责任。只盯索引不看调用场景，问题会反复出现。
