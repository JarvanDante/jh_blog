---
author: "jarvan"
title: "PHP 项目的可观测性建设：日志、指标、链路追踪一体化"
date: 2020-10-31T10:00:00+08:00
description: "PHP 项目的可观测性建设：日志、指标、链路追踪一体化"
draft: false
hideToc: false
enableToc: true
...
过去我也只看日志，后来发现：没有指标和 trace，定位速度会被严重拖慢。

## 最小可观测闭环

- **日志**：结构化 JSON，统一 `trace_id`
- **指标**：QPS、错误率、P95、队列堆积
- **追踪**：跨服务 span（网关 -> 应用 -> DB）

## 日志字段建议

```json
{"service":"order-api","trace_id":"...","route":"/order/create","uid":123,"cost_ms":43}
```
