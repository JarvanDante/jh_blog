---
author: "jarvan"
title: "PHP 在微服务架构中的定位：从业务胶水到核心交易服务"
date: 2020-06-18T10:00:00+08:00
description: "PHP 在微服务架构中的定位：从业务胶水到核心交易服务"
draft: false
hideToc: false
enableToc: true
...
在很多团队里，PHP 经常被贴上“只适合做 CRUD”的标签。但在真实业务中，PHP 只要边界划分清晰，一样能承担核心交易链路。

## 我对 PHP 服务边界的划分

- **强事务链路**：订单、支付回调、账务落库（PHP + MySQL）
- **高吞吐异步链路**：消息消费、批处理（PHP + MQ + Worker）
- **高性能读链路**：缓存聚合、只读 API（PHP + Redis）

关键不是语言，而是职责：把“核心一致性”留在最稳定链路，把“可重试任务”放进异步系统。
...
{
    public function __construct(private OrderRepo $repo, private OutboxRepo $outbox) {}

    public function handle(CreateOrderCmd $cmd): string
    {
        return DB::transaction(function () use ($cmd) {
            $orderNo = OrderNo::next();
            $this->repo->create($orderNo, $cmd->uid, $cmd->amount);
            $this->outbox->append('order.created', ['order_no' => $orderNo]);
            return $orderNo;
        });
    }
}
...
这段代码的重点不是“优雅”，而是保证订单创建与事件写入同事务提交，后续才能可靠进入异步链路。

## 什么时候不该让 PHP 扛核心链路？

如果场景是极端低延迟撮合、超高并发计算密集型任务，PHP 不是最优。我的策略是：PHP 负责业务编排和一致性，计算热点交给 Go/Rust 服务。

## 小结

PHP 不是“做不了核心”，而是要明确：哪些链路由 PHP 承担、哪些链路交给更合适的组件。架构师的工作是做边界，不是做信仰。
