---
author: "jarvan"
title: "支付系统的可靠性设计：回调幂等、重试与对账补偿"
date: 2020-09-01T10:00:00+08:00
description: "支付系统的可靠性设计：回调幂等、重试与对账补偿"
draft: false
hideToc: false
enableToc: true
...
支付链路要的是“少承诺，多校验”。尤其回调处理，最忌讳一次请求定生死。
...
重复回调只允许 `PAYING -> SUCCESS` 一次，其他直接 ACK。

## 代码片段

```php
if (!$this->signVerifier->verify($payload)) {
    return 'FAIL';
}
$order = $repo->lockByOrderNo($payload['order_no']);
if ($order->isSuccess()) return 'OK';
$order->markSuccess($payload['trade_no']);
```
