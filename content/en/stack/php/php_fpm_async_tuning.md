---
author: "jarvan"
title: "PHP 高并发实战：从 FPM 调优到异步化改造"
date: 2020-07-03T10:00:00+08:00
description: "PHP 高并发实战：从 FPM 调优到异步化改造"
draft: false
hideToc: false
enableToc: true
...
这篇不讲“万能参数”，只讲我线上真正踩过的坑：CPU 还没满，QPS 却上不去。

### 第一步：先看瓶颈在哪

我会先抓 4 个指标：

1. Nginx upstream 响应时间
2. PHP-FPM active/idle 进程数
3. MySQL 慢查询与连接池
4. Redis 命中率

很多时候不是 FPM 不够，是 SQL 慢、外部接口慢把 worker 卡死。
...
`pm.max_requests` 很关键，可以缓解长跑进程内存碎片。

### 第三步：同步改异步

把“用户请求路径上的非必要动作”全部剥离：

- 发站内信
- 刷报表
- 推送第三方

改成 MQ 后，请求 RT 通常能明显下降。
...
并发优化的主线是：先定位瓶颈，再调 FPM，最后做异步化。顺序反了，基本都白忙。
